...
def expl(x,y,img):
    for f in range(100):    
        screen.blit(img, (x, y))
        show_health(font,p1.helth,600,20,'PLAYER 1 : ',red)
        show_health(font,p2.helth,20,20, 'PLAYER 2 : ',green)
...
def collision (x,y,x1,Y1,stat):
    if not stat:
        ...
        if distance < 27:
            ...
            return False      
def show_health(font,helth,x, y,nom_player,color):
...
exp = pygame.image.load('explode.png')
green = (0, 255, 0)
red = (255, 0, 0)
...
while run :
    ...
    while in_game :
        ...
        if collision(p1.x,p1.y,m2.x-50,m2.y,m2.stat):
            bulletSound = mixer.Sound("explosion.wav")
            bulletSound.play()
            p1.helth=p1.helth-1
            m2.stat=True
            expl(p1.x-10,p1.y,exp)
        if collision(p2.x,p2.y+25,m1.x,m1.y,m1.stat):
            bulletSound = mixer.Sound("explosion.wav")
            bulletSound.play()
            p2.helth=p2.helth-1
            m1.stat=True
            expl(p2.x+10,p2.y,exp)
        if p1.helth <= 0:
    ...
    while end_game :
        ...
        start_text = start_font.render('PRESS ANY BUTTON TO RESTART', True, (255, 255, 255))
